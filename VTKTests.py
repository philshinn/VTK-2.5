...
def testGrammarStates(sm):
    ...
    text = 'yes'
    meanings = grammarState.parseInput(text,sm)
    if dbg:
...
def testDBStates(sm):
    dbg = True
    stateName = 'MakeAccountDB'
    print("Info: in testDBStates:1 stateName=",stateName)
    dbState =  sm.objects[sm.objectName2Index[stateName]]
    print("Info: in testDBStates:2",dbState.dump())
    dbName = 'VTK_DB.db'
...
def runtests(sm):
    ...
    #testPrompts(sm)
    testDBStates(sm)
    sm.run('StartDBExample')

if __name__ == "__main__":
    dbg = True
    sm = StateMachine()                                                                                                     # create an instance of general state machine
    sm.readDrawIOXMLFile("VTK 2.6 DB.xml")                                                                                    # read in the data from the GUI of a particular design

    #sm.readDrawIOXMLFile("VTK 2.5.xml")                                                                                    # read in the data from the GUI of a particular design
                                                                               # read in the data from the GUI of a particular design
    if dbg:
